...
int DetermineTowerBackgroundv1::fill_energy_vectors(PHCompositeNode *topNode, Jet::SRC src)
{
  ...
  for ( int ich = 0; ich < ntowers; ich++ )
  {
    ...
    double comp_z0     = std::sinh( comp_eta ) * tower_r;
    double comp_z      = comp_z0 - m_vtxz;
    double comp_eta_corr = std::asinh( comp_z / tower_r );
...
int DetermineTowerBackgroundv1::init_event(PHCompositeNode *topNode)
{
  if ( first )
  {
    ...
    m_emcal_tower_eta.resize(m_num_eta_ihcal, 0);
    m_ihcal_tower_eta.resize(m_num_eta_ihcal, 0);
    m_ohcal_tower_eta.resize(m_num_eta_ihcal, 0);
  ...
  m_emcal_tower_eta.assign(m_num_eta_ihcal, 0);
  m_ihcal_tower_eta.assign(m_num_eta_ihcal, 0);
  m_ohcal_tower_eta.assign(m_num_eta_ihcal, 0);
...
int DetermineTowerBackgroundv1::process_event(PHCompositeNode *topNode)
{
  ...
  for ( size_t i = 0; i < ktjets -> size(); i++ )
  {
    ...
    for ( const auto & comp : this_jet -> get_comp_vec() )
    {
      ...
      double comp_e = ti -> get_energy();
      double comp_px = comp_eT * std::cos( comp_phi );
      double comp_py = comp_eT * std::sin( comp_phi );
      double comp_pz = comp_eT * std::sinh( comp_eta );
      if ( comp_e < 0 )
      {  
        // make energy = +1 MeV for purposes of clustering
        const double e_ratio = 0.001 / comp_e;
        comp_e*= e_ratio;
        comp_px*= e_ratio;
        comp_py*= e_ratio;
        comp_pz*= e_ratio;
    ...
    auto * fj_jet = new Jetv2();
    fj_jet -> set_px( fj_px );
    fj_jet -> set_py( fj_py );
    fj_jet -> set_pz( fj_pz );
    fj_jet -> set_e( fj_e );

    seed_pt = fj_jet -> get_pt();

    this_jet -> set_property( m_idx_seedD, seed_pt );
  ...
  if ( n_eta_emcal > 0 )
  {
    avg_avg_emcal_energy /= n_eta_emcal;
  }
  if ( n_eta_ihcal > 0 )
  {
    avg_avg_ihcal_energy /= n_eta_ihcal;
  }
  if ( n_eta_ohcal > 0 )
  {
    avg_avg_ohcal_energy /= n_eta_ohcal;
  }
  ...
  if ( !tr_cemc )
  {
    std::cout << "DetermineTowerBackgroundv1::process_event: WARNING: cannot find " << m_cemc_rho_node << ", setting rho_emcal = 0" << std::endl;
    exit(-1);
  }
  else
  {
    rho_emcal = tr_cemc -> get_rho();
  }
  auto * tr_ihcal = findNode::getClass<TowerRhov1>(topNode, m_ihcal_rho_node);
  if ( !tr_ihcal )
  {
    std::cout << "DetermineTowerBackgroundv1::process_event: WARNING: cannot find " << m_ihcal_rho_node << ", setting rho_ihcal = 0" << std::endl;
    exit(-1);
  }
  else
  {
    rho_ihcal = tr_ihcal -> get_rho();
  }
  auto * tr_ohcal = findNode::getClass<TowerRhov1>(topNode, m_ohcal_rho_node);
  if ( !tr_ohcal )
  {
    std::cout << "DetermineTowerBackgroundv1::process_event: WARNING: cannot find " << m_ohcal_rho_node << ", setting rho_ohcal = 0" << std::endl;
    exit(-1);
  }
  else
  {
    rho_ohcal = tr_ohcal -> get_rho();
  }

  // reset the UE density
  for (int ieta = 0; ieta < m_num_eta_ihcal; ieta++ )
  {
    double ue_emcal = m_ue_density.at(0).at(ieta);
