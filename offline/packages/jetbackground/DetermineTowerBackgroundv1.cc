...
int DetermineTowerBackgroundv1::get_zvrtx(PHCompositeNode *topNode)
{
  ...
  if ( !vertexmap )
  {
    std::cout << "DetermineTowerBackgroundv1::get_zvrtx - Fatal Error - GlobalVertexMap node is missing. Please turn on the do_global flag in the main macro in order to reconstruct the global vertex." << std::endl;
    exit(1);
  }
  ...
  if ( vertexmap->empty() )
  {
    if (Verbosity() > 0)
    {
      std::cout << "DetermineTowerBackgroundv1::get_zvrtx - empty vertex map, continuing as if zvtx = 0" << std::endl;
    }
    return Fun4AllReturnCodes::EVENT_OK;
  }
  ...
  if ( std::isnan(m_vtxz) || std::abs(m_vtxz) > 1e3 )
  {
    ...
    {
      once = false;
      std::cout << "DetermineTowerBackgroundv1::get_zvrtx - WARNING - vertex is NAN. Continue with zvtx = 0 (further vertex warning will be suppressed)." << std::endl;
    }
    m_vtxz = 0;
  }
  ...
  if (Verbosity() > 1)
  {
    std::cout << "DetermineTowerBackgroundv1::get_zvrtx - zvtx = " << m_vtxz << std::endl;
  }
  return Fun4AllReturnCodes::EVENT_OK;
...
int DetermineTowerBackgroundv1::fill_energy_vectors(PHCompositeNode *topNode, Jet::SRC src)
{
  ...
  for ( int ich = 0; ich < ntowers; ich++ )
  {
    ...
    if (jeta < 0 || jphi < 0)
    {
      if (Verbosity() > 3)
      {
        std::cout << "DetermineTowerBackgroundv1::fill_energy_vectors - tower with eta/phi = " << comp_eta << " / " << comp_phi << " is out of bounds, skipping" << std::endl;
      }
      continue;
    }
  ...
  if (Verbosity() > 2)
  {
    std::cout << "DetermineTowerBackgroundv1::fill_energy_vectors - finished filling energy vectors for src = " << static_cast<int>(src) << std::endl;
...
int DetermineTowerBackgroundv1::get_psi2(PHCompositeNode *topNode)
{
  m_psi2 = 0.0;
  ...
  else if ( m_psi2_mode == 1 )
  {
    ...
    for ( int iphi = 0; iphi < m_num_phi_ihcal; iphi++ )
    {
      ...
      // check the flow mask in this case 
      for ( int ieta = 0; ieta < m_num_eta_ihcal; ieta++ )
      {
        if ( !m_emcal_flow_mask[ieta][iphi] )
        {
          double w = 1.0;
          if ( m_renormalize_eta_strip )
          {
            w = m_ue_density.at(0).at(ieta);
          }
          if ( w == 0 )
          { 
            // skip bad energies
          ...
          en_emcal += m_emcal_energy[ieta][iphi] / w;
          n_emcal_towers++;
        }
        if ( !m_ihcal_flow_mask[ieta][iphi] )
        {
          double w = 1.0;
          if ( m_renormalize_eta_strip )
          {
            w = m_ue_density.at(1).at(ieta);
          }
          if ( w == 0 )
          { 
            // skip bad energies
          ...
          en_ihcal += m_ihcal_energy[ieta][iphi] / w;
          n_ihcal_towers++;
        }
        if ( !m_ohcal_flow_mask[ieta][iphi] )
        {
          double w = 1.0;
          if ( m_renormalize_eta_strip )
          {
            w = m_ue_density.at(2).at(ieta);
          }
          if ( w == 0 )
          { 
            // skip bad energies
          ...
          en_ohcal += m_ohcal_energy[ieta][iphi] / w;
          n_ohcal_towers++;
        }
      }
      ...
      if ( n_emcal_towers > 0 )
      {
        en_emcal /= n_emcal_towers;
      }
      if ( n_ihcal_towers > 0 )
      {
        en_ihcal /= n_ihcal_towers;
      }
      if ( n_ohcal_towers > 0 )
      {
        en_ohcal /= n_ohcal_towers;
      }

      double en = en_emcal + en_ihcal + en_ohcal;
    ...
    if ( Verbosity() > 0 )
    {
      std::cout << "DetermineTowerBackgroundv1::GetPsi: flow extracted from tower map, setting Psi2 = " << m_psi2 << " ( " << m_psi2 / M_PI << " * pi ) " << std::endl;
    }
  }
  ...
  { 
    ...
    if ( Verbosity() > 0 )
    {
      std::cout << "DetermineTowerBackgroundv1::GetPsi: flow extracted from EventHeader, setting Psi2 = " << m_psi2 << " ( " << m_psi2 / M_PI << " * pi ) " << std::endl;
    }
  ...
  { 
    ...
    if (Verbosity() > 0)
    {
      std::cout << "DetermineTowerBackgroundv1::process_event: flow extracted from sEPD, setting Psi2 = " << m_psi2 << " ( " << m_psi2 / M_PI << " * pi ) " << std::endl;
    }

  }
  else
  {
    // this should never happen since we check the validity of m_psi2_mode in the Init function, but just in case
    std::cout << "DetermineTowerBackgroundv1::GetPsi - invalid psi2_mode = " << m_psi2_mode << ", exiting" << std::endl;
    exit(1);
  }
...
int DetermineTowerBackgroundv1::get_v2(PHCompositeNode *topNode)
{
  ...
  else if ( m_flow_mode == 1)
  {
    ...
    if ( Verbosity() > 0 )    
    {
      std::cout << "DetermineTowerBackgroundv1::Getv2: flow calculated from tower map with event plane correction, setting v2 = " << m_v2 << std::endl;
    }
  ...
  else if ( m_flow_mode == 2 )
  {
    ...
    if ( Verbosity() > 0 )
    {
      std::cout << "DetermineTowerBackgroundv1::Getv2: flow extracted from centrality with event plane correction, setting v2 = " << m_v2 << std::endl;
    }
  
  }
  else 
  {
    std::cout << "DetermineTowerBackgroundv1::Getv2 - invalid flow_mode = " << m_flow_mode << ", exiting" << std::endl;
    exit(1);
  }

  if ( Verbosity() > 1 )
  {
    std::cout << "DetermineTowerBackgroundv1::Getv2 - finished calculating v2, v2 = " << m_v2 << std::endl;
  }

  return Fun4AllReturnCodes::EVENT_OK;
}

int DetermineTowerBackgroundv1::init_event(PHCompositeNode *topNode)
{
  if ( first )
  {
    if (Verbosity() > 0)
    ...
    m_emcal_flow_mask.resize(m_num_eta_ihcal, std::vector<bool>(m_num_phi_ihcal, false));
    m_ihcal_flow_mask.resize(m_num_eta_ihcal, std::vector<bool>(m_num_phi_ihcal, false));
    m_ohcal_flow_mask.resize(m_num_eta_ihcal, std::vector<bool>(m_num_phi_ihcal, false));
  ...
  m_emcal_flow_mask.assign(m_num_eta_ihcal, std::vector<bool>(m_num_phi_ihcal, false));
  m_ihcal_flow_mask.assign(m_num_eta_ihcal, std::vector<bool>(m_num_phi_ihcal, false));
  m_ohcal_flow_mask.assign(m_num_eta_ihcal, std::vector<bool>(m_num_phi_ihcal, false));
  ...
  if ( m_seed_type == 0 )
  {
    ...
    {
      m_eta_bin_edges.at(i) = std::asinh( ( m_z0_bin_edges.at(i) - m_vtxz )/ m_ihcal_r );
      m_eta_bin_edges.at(i+1) = std::asinh( ( m_z0_bin_edges.at(i+1) - m_vtxz )/ m_ihcal_r );
    }
  }
...
int DetermineTowerBackgroundv1::process_event(PHCompositeNode *topNode)
{
  if ( Verbosity() > 1 )
  {
    std::cout << "DetermineTowerBackgroundv1::process_event - starting process_event" << std::endl;
  }
  ...
  // get indices for jet properties
  ...
  for ( size_t i = 0; i < ktjets -> size(); i++ )
  {
    ...
    if ( this_pt < m_seed_jet_pt )
    {
      if ( Verbosity() > 2 )
      {
        std::cout << "DetermineTowerBackgroundv1::process_event: " << \
      ...
      this_jet -> set_property( m_idx_seed_itr, 0);
      continue; // no longer consider this jet 
    }
    ...
    {  
      ...
      for ( const auto & comp : this_jet -> get_comp_vec() )
      {
        ...
        if ( Verbosity() > 4 )
        {
          std::cout << "DetermineTowerBackgroundv1::process_event: --> --> constituent in layer " << comp.first << " at ieta / iphi = " << comp_ieta << " / " << comp_iphi << ", filling map with key = " << key << " and ET = " << comp_eT << std::endl;
        }
        ...
        if ( Verbosity() > 4 )
        {
          std::cout << "DetermineTowerBackgroundv1::process_event: --> --> constituent in layer " << comp.first << " at ieta / iphi = " << comp_eta << " / " << comp_phi << ", filling map with key = " << ckey << " and ET = " << comp_eT << std::endl;
          std::cout << "DetermineTowerBackgroundv1::process_event: --> --> map at key = " << ckey << " now has ET = " << constituent_ETsum[ckey] << std::endl;
        }
      ...
      for ( const auto & map_iter : constituent_ETsum )
      {
        if ( Verbosity() > 4 )
        {
          std::cout << "DetermineTowerBackgroundv1::process_event: --> --> map has key # " << map_iter.first << " and ET = " << map_iter.second << std::endl;
        }
        n_supercomps++;
      ...
      // skip seeds that do not pass D and max constituent ET criteria
      if ( seed_D <  m_seed_jet_D ||  max_supercomp_ET < m_seed_max_const )
      {
        if ( Verbosity() > 2 )
        {
          std::cout << "DetermineTowerBackgroundv1::process_event: --> jet with pt / eta / phi = " << this_pt << " / " << this_jet -> get_eta() << " / " << this_jet -> get_phi() << " below seed D or max const threshold ( D = " << seed_D << ", max const ET = " << max_supercomp_ET << " ), skipping" << std::endl;
        }
        continue; // no longer consider this jet
      }
      ...
      if ( Verbosity() > 3 )
      {
        std::cout << "DetermineTowerBackgroundv1::process_event: --> jet has < ET > = " << sum_supercomp_ET << " / " << n_supercomps << " = " << mean_supercomp_ET << ", max-ET = " << max_supercomp_ET << ", and D = " << seed_D << std::endl;
      }
    ...
    if ( m_seed_type == 1 )
    {
      ...
      if ( Verbosity() > 2 )
      {
        std::cout << "DetermineTowerBackgroundv1::process_event: possible seed" << \
          " jet with pt / eta / phi = " << this_pt << " / " << this_jet -> get_eta() << " / " << this_jet -> get_phi() << \
    ...
    if ( Verbosity() > 2 )
    {
      std::cout << "DetermineTowerBackgroundv1::process_event: --> jet has value = " << this_val << std::endl;
    }
  ...
  if ( m_n_omit_seeds < 0 )
  {
    ...
    {
      // all seeds in seed_val_pairs have already passed the relevant threshold, so accept them all
      accepted_seed_indices.push_back( pair.first );
    }
  }
  else 
  {
    // only accept up to m_n_omit_seeds seeds, starting from the highest value
    // sort jets by value
  ...
  // reloop back over accepted seeds to update masks
  ...
  for ( const auto & i : accepted_seed_indices )
  {
    ...
    if ( m_seed_type == 0 )
    {
      if ( Verbosity() > 1 )
      {
        std::cout << "DetermineTowerBackgroundv1::process_event: --> omitting jet with pt / eta / phi = " << this_jet -> get_pt() << " / " << this_jet -> get_eta() << " / " << this_jet -> get_phi() << " from seed consideration since it is among the " << m_n_omit_seeds << " jets with highest D value " << val << std::endl;
      }
    ...
    if ( m_seed_type == 1 )
    {
      if ( Verbosity() > 1 )
      {
        std::cout << "DetermineTowerBackgroundv1::process_event: --> omitting jet with pt / eta / phi = " << this_jet -> get_pt() << " / " << this_jet -> get_eta() << " / " << this_jet -> get_phi() << " from seed consideration since it is among the " << m_n_omit_seeds << " jets with highest pT value " << val << std::endl;
      }
      this_jet -> set_property( m_idx_seed_itr, 2.0 );
    }
    if ( Verbosity() > 1 )
    {
      std::cout << "DetermineTowerBackgroundv1::process_event: --> accepted seed jet with pt / eta / phi = " << this_jet -> get_pt() << " / " << this_jet -> get_eta() << " / " << this_jet -> get_phi() << " and value = " << i << "( nAcc/nOmitted = " << n_accepted_seeds << " / " << m_n_omit_seeds << " )" << std::endl;
    }

    if ( m_seed_exclusion_DR <= 0.0f )
    {
      // this means that DR exclusion is disabled, 
      // in this case we only want to mark the towers that are constituents of the seed jets as masked, and not any additional towers within a given DR
      if ( Verbosity() > 2 )
      {
        std::cout << "DetermineTowerBackgroundv1::process_event: --> DR exclusion disabled, only marking towers that are direct constituents of seed jets as masked" << std::endl;
      }
      // loop over constituents of this jet and mark them as masked
      for ( const auto & comp : this_jet -> get_comp_vec() )
      {
        ...
        // no need to worry about geom correction since we are in tower geom 
        ...
        if ( Verbosity() > 4 )
        {
          std::cout << "DetermineTowerBackgroundv1::process_event: --> --> marking tower in layer " << comp.first << " at ieta / iphi = " << comp_ieta << " / " << comp_iphi << " ( jeta / jphi = " << jeta << " / " << jphi << " ) as masked since it is a constituent of an accepted seed jet" << std::endl;
        }
      
      }
    } // end of if const exlcusion mode
    else 
    {
      // here the m_seed_exclusion_DR is a set value and we exclude all towers within DR of the jet axis
      // this is annoying because we have to loop over all towers to apply the flow mask
      double this_phi  = this_jet -> get_phi();
      double this_eta  = this_jet -> get_eta();
      for ( const auto & src : m_jetsrcs )
      {
        double tower_r = 0.0;
        m_towerinfos = nullptr;
        m_towergeom = nullptr;
        ...
        for ( int ich = 0; ich < ntowers; ich++ )
        {
          ...
          // need to apply geom corr here to get dR
          double tower_z0   = sinh( tg -> get_eta() ) * tower_r;
          double comp_z     = tower_z0 - m_vtxz;
          double comp_eta   =  asinh( comp_z / tower_r ); 
          double comp_phi   = atan2( tg -> get_center_y(), tg -> get_center_x() );

          // we are in tower geom binning
          const int jeta = comp_ieta;
          const int jphi = comp_iphi;

          double dphi = comp_phi - this_phi;
          if ( dphi > M_PI )
          {
            dphi -= 2 * M_PI;
          }
          ...
          double deta = comp_eta - this_eta;
          double dR = sqrt( dphi * dphi + deta * deta );

          // mask accoridingly
          if ( dR < m_seed_exclusion_DR )
          {
            if ( Verbosity() > 4 )
            {
              std::cout << "DetermineTowerBackgroundv1::process_event: --> --> marking tower in layer " << src << " at ieta / iphi = " << comp_eta << " / " << comp_phi << " ( jeta / jphi = " << jeta << " / " << jphi << " ) as masked since it is within DR = " << dR << " of the accepted seed jet axis" << std::endl;
            }
            if ( src == Jet::SRC::HCALIN_TOWERINFO )
            {
              m_ihcal_mask.at(jeta).at(jphi) = true;
            ...
            if ( src == Jet::SRC::CEMC_TOWERINFO_RETOWER || src == Jet::SRC::CEMC_TOWERINFO )
            {
              m_emcal_mask.at(jeta).at(jphi) = true;
            }
          }
  ...
  // keep track of which eta strips need to be fully masked in this case so that we can skip them in the flow later
  std::set<int> eta_strips_to_be_fully_masked{};
  for ( const auto & src : m_jetsrcs )
  {
    m_towerinfos = nullptr;
    m_towergeom = nullptr;
  ...
  // if do_reweight is not on then we also mark all the eta strips that have
  /// a single bad tower as fully masked 
  if ( !m_do_reweight )
  {
    for ( const auto & ieta : eta_strips_to_be_fully_masked )
    {
      // this only applies to the flow mask
      for ( int iphi = 0; iphi < m_num_phi_ihcal; iphi++ )
      {
        m_ihcal_flow_mask.at(ieta).at(iphi) = true;
        m_emcal_flow_mask.at(ieta).at(iphi) = true;
        m_ohcal_flow_mask.at(ieta).at(iphi) = true;
      }
      if ( Verbosity() > 2 )
      {
        std::cout << "DetermineTowerBackgroundv1::process_event: --> marking full eta strip at ieta = " << ieta << " as masked since it contains at least one bad tower and do_reweight is false" << std::endl;
      }
    }
  }
  // At this point the seed masks are fully set
  // bad mask: all bad towers + all seed exclusion towers 
  // flow mask if exclude full eta && do_reweight : all eta strips with seeds + all bad towers
  // flow mask if exclude full eta && !do_reweight : all eta strips with seeds + all eta strips with bad towers
  // flow mask if !exclude full eta && do_reweight : same as bad mask
  ...
  for (int ieta = 0; ieta < m_num_eta_ihcal; ieta++ )
  {
    ...
    if ( Verbosity() > 2 )
    {
      std::cout << "DetermineTowerBackgroundv1::process_event: for eta bin " << ieta << ", avg energy in emcal / ihcal / ohcal is " << avg_emcal_energy << " / " << avg_ihcal_energy << " / " << avg_ohcal_energy << " calculated from " << n_emcal_towers << " / " << n_ihcal_towers << " / " << n_ohcal_towers << " towers" << std::endl;
    }
  ...
  // Get psi2
  get_psi2(topNode);
  // check psi2 has physical bounds
  if ( !std::isfinite(m_psi2) || std::abs(m_psi2) > M_PI/2.0 )
  {
    if (Verbosity() > 0)
    {
      std::cout << "DetermineTowerBackgroundv1::GetPsi - WARNING Psi2 is non-finite (NaN or Inf), setting Psi2 = 0." << std::endl;
    }
  ...
  {
    // only attempt if we have a valid psi2 otherwiseits a waste of time
    get_v2(topNode);
  }
  // check v2 has physical bounds
  if ( !std::isfinite(m_v2) || std::abs(m_v2) > 0.5 )
  {
    if (Verbosity() > 0)
    {
      std::cout << "DetermineTowerBackgroundv1::Getv2 - WARNING v2 is non-finite (NaN or Inf), setting v2 = 0 and flow failure flag to true." << std::endl;
    }
    m_is_flow_failure = true;
    m_v2 = 0.0;
    m_psi2 = 0.0;
  }
  // this is redundant but fuck it
  if ( m_is_flow_failure )
  {
    m_v2 = 0.0;
    m_psi2 = 0.0;
    if ( Verbosity() > 0 )
    {
      std::cout << "DetermineTowerBackgroundv1::process_event: flow failure detected, setting v2 to 0 and proceeding with background calculation without flow modulation" << std::endl;
    }
  }
  ...
  if ( Verbosity() > 0 )
  {
    std::cout << "DetermineTowerBackgroundv1::process_event: done calculating background, EMCal UE density = [ ";
    for (int ieta = 0; ieta < m_num_eta_ihcal; ieta++ )
    {
  ...
  if ( Verbosity() > 0 )
  {
    std::cout << "DetermineTowerBackgroundv1::process_event: filled TowerBackground node with background information" << std::endl;
  }
...
int DetermineTowerBackgroundv1::CreateNode(PHCompositeNode *topNode)
{
...
void DetermineTowerBackgroundv1::FillNode(PHCompositeNode *topNode , const std::string & name)
{
